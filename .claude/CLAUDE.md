...
This project replicates Uber's **TAROT (TARgeting and Observation Toolkit)** — an incentive targeting and optimization system — using [encore.dev](https://encore.dev/docs/go) with Go.

TAROT solves the problem of **optimally allocating incentives** (promotions, discounts, rewards) to users under budget constraints. It models this as a **Multiple Knapsack Problem** where each budget is a "knapsack" and each user-incentive pair is an item with predicted value.

## Architecture

The system consists of the following encore.dev services, mapped from Uber's original architecture:

| Service | Uber Original | Role |
|---|---|---|
| `orchestrator` | Orchestrator (Go + Cadence) | Central coordinator — triggers targeting runs, fans out to other services |
| `segmentation` | Segmentation Platform (Spark) | Defines and resolves user cohorts/segments |
| `mlgateway` | ML Gateway (Spark + Cassandra) | Serves ML predictions (incentive response probabilities) |
| `budgeting` | Budgeting Platform (Go) | Manages configured budgets per incentive program |
| `pacer` | Budget Pacer (Go library) | Tracks remaining budgets and pacing over time |
| `optimizer` | Multi-lever Optimizer (Ray) | Solves the multiple knapsack allocation problem |
| `domain` | Domain Services (Go) | Assigns resolved incentives to users |
| `config` | Configuration UI Tool | CRUD API for incentive use-case configuration |

### Targeting Run Flow (triggered by cron)

1. **Cron** triggers the Orchestrator
2. Orchestrator fetches **users** from Segmentation
3. Orchestrator fetches **ML predictions** from ML Gateway
4. Orchestrator fetches **remaining budgets** from Pacer
5. Orchestrator sends all data to **Optimizer** (multiple knapsack solver)
6. Orchestrator sends optimized assignments to **Domain Services**

### Communication Patterns

- **Encore APIs** (RPC): service-to-service synchronous calls (replaces gRPC)
- **Pub/Sub topics**: async events between services (replaces Kafka)
- **Cron jobs**: periodic targeting triggers
- **SQL databases**: per-service data stores (replaces Cassandra/Docstore)

## Key References

- **Paper**: [TAROT: Targeted and Automated Incentive Optimization](https://arxiv.org/abs/2407.19078)
- **Blog**: [Solving Multiple Knapsack at Scale](https://www.uber.com/br/pt-br/blog/solving-multiple-knapsack/)
- **Encore docs**: https://encore.dev/docs/go
...
When assisting, adopt these combined perspectives:
- **Backend Architect**: Scalable system design, database architecture, performance (<200ms p95), reliability (99.9%+ uptime), security-first
- **Software Architect**: Domain modeling with bounded contexts, justified complexity, named trade-offs, ADRs for key decisions, reversible choices over "optimal" ones
- **UX Architect**: Clean API design, developer-friendly interfaces, consistent naming conventions

### Code Style

- Follow standard Go conventions (gofmt, effective Go)
- Use domain-driven design — each service owns its data and exposes clean APIs
- Keep services loosely coupled; communicate via APIs and pub/sub
- Write table-driven tests
- Use meaningful error messages with context wrapping
